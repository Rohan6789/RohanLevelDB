...
//Begin New implementation of put method that takes array of skip list as input - RK
Status DBImpl::PutSkipArray(const WriteOptions& o,
		const std::vector<std::vector<std::vector<std::string>>> &sV) {
	return DB::PutSkipArray(o, sV);
}
...
//Begin new implementation of put method that takes array of skip list as input - RK
Status DB::PutSkipArray(const WriteOptions& opt,
		const std::vector<std::vector<std::vector<std::string>>> &skipV) {
	Status sa;
	std::vector<std::vector<std::string>> skip;
	WriteBatch batchSkip;
	cout <<"DB-PutSkipArray" << endl;
	cout << "skipV size ="  << skipV.size() << endl;
	for (int iList=0; iList<skipV.size(); iList++) {
		skip = skipV[iList];
		cout << "skip size =" << skip.size() << endl;

		for (int jList = 0; jList<skip.size(); jList++) {
			if (sa.ok()) {
				batchSkip.Clear();
				batchSkip.Put(skip[jList][0],skip[jList][1]);
				sa = Write(opt, &batchSkip);
			}
			else {
				cerr << "Insert Failed for Skip List# =" << iList << "Key-Value Pair # =" << jList << endl;
			}
		}
		skip.clear();
	}
...
//End new implementation of put method that takes array of skip list as input - RK
